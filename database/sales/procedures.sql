...
--     IF NOT FOUND THEN
--         RAISE EXCEPTION 'Invalid: customer_id or other details are wrong';
--     END IF;
...
--     SELECT SUM(sub_total) INTO v_sub_total FROM sales.sale_items WHERE sale_id = g_sale_id;
...
-- CREATE OR REPLACE PROCEDURE sales.cancel_item(
--     g_sale_id INT,
--     g_donut_id INT,
--     g_reason reasons,
--     v_new_total OUT DECIMAL
-- )
-- LANGUAGE plpgsql
-- AS $$
-- DECLARE
...
--     v_order_count INT;
-- BEGIN
--     SELECT COUNT(sale_id) INTO v_order_count FROM sales.sale_items WHERE sale_id = g_sale_id;
...
--         RAISE EXCEPTION 'Order count 1 you need to use full cancel sale';
--     END IF;
...
--     FROM sale_items WHERE donut_id = g_donut_id AND sale_id = g_sale_id
...
--     SELECT SUM(sub_total) INTO v_new_total FROM sales.sale_items WHERE sale_id = g_sale_id;
...
-- CREATE OR REPLACE PROCEDURE sales.cancel_sale(g_sale_id INT, g_reason reasons)
-- LANGUAGE plpgsql
-- AS $$
-- DECLARE
--     r RECORD;
